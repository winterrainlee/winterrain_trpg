# 📁 **상태 보기 모듈 (Status View Module)** Ver 1.6 (251228)
...
`wf.pm_요약_한줄` 생성 규칙:
- `wf.pm.ct`가 비었으면: `"미설정"`
- 아니면: `{wf.pm.ct}`를 기본으로 출력하고, `wf.pm.tg`가 있으면 카드 종류에 맞는 항목을 최대 3개까지 덧붙인다.
  - 표기 형식: `{카드} / {항목1}, {항목2}, {항목3}`
  - 카드별 토글 표기(키가 없으면 생략):
    - 추리·수사: `범인 보장={culprit}`, `PC 역할={pc_role}`, `결말={ending}`, `초자연={supernatural}`, `우연={luck}`
    - 정치: `잔혹도={cruelty}`, `암투 비중={shadow}`, `전쟁={war}`, `여론 영향={public}`
    - 일상: `현실성={realism}`, `사건 강도={drama}`, `로맨스={romance}`, `타임스킵={timeskip}`
    - 로맨스: `결말={ending}`, `관계 속도={pace}`, `삼각관계={triangle}`, `수위={maturity}`

> ⚠️ 상태창은 JSON 키를 그대로 노출하지 않는 게 원칙이므로, 위 항목명(한글 라벨)로만 출력한다.
