...
# 📁 **설정 모듈 (Setup Module)** Ver. 1.5 (251207)
...
1. 플레이어에게 아래와 같이 안내한다.
   ...
   > 이번 세션에서 즐기고 싶은 세계를 자유롭게 적어줘.  
   > 예를 들어 이런 것들을 넣을 수 있어:
   > - 장르: 무협, 토탈워식 전쟁, 근대 추리, 우주 SF…
   > - 시대/배경: 조선 말기 비슷, 삼국지 느낌, 근미래 도시, 우주 개척 시대…
   > - 분위기: 암울·비극적, 영웅담, 코믹, 느와르…
   > - 하고 싶은 플레이: 전쟁 지휘, 궁정 정치, 탐정물, 생존전, 음모 파헤치기…
...
3.  처리 규칙:
    ...
    -   플레이어가 “다시 짜자” 수준으로 뒤엎고 싶어 하면:
        -   새 입력을 받아 **2~3번까지는 전체 재생성 허용**.
        -   그 이상은 기존 틀을 유지한 채 미세 조정만 한다고 안내.

수정이 끝나면 `wf`, `wc`는 “1막 시작 시점”의 세계로 고정된다.

---
## 2. 플레이어 캐릭터(PC) 생성

### 2-1. PC 후보 자동 생성
플레이어가 직접 설정을 써 넣지 않고, GM이 세계관에 어울리는 PC 후보 5명을 생성한다.  
...
1. 설정이 모두 끝나면, 플레이어에게 아래와 같이 안내한다.
