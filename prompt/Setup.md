...
# 📁 **설정 모듈 (Setup Module)** Ver. 1.6 (251228)
...
"wf": {
  ...
  "cc": "",  // core_conflict
  "pm": {
    "ct": "", // promise card type
    "tg": {},  // toggles
    "pr": []   // promises (player-facing lines)
  }
}
```
...
3.  처리 규칙:
  - “OK/진행/좋다/괜찮다” 등 긍정 → `wf`, `wc` 확정 후 **장르 약속 카드 단계로 진행**.
  - 수정 텍스트가 오면:
...
---
### 1-4. 장르 약속 카드 확정: `wf.pm`

플레이어가 기대하는 장르 상식과 실제 전개가 어긋나지 않도록, “이번 세션에서 보장할 것”을 짧게 고정한다.

#### 1) 카드 자동 선택(추가 질문 최소화)
- `wf.ge`의 키워드로 아래 중 하나를 자동 선택한다.
  - 추리/수사/탐정/미스터리 → **추리·수사**
  - 정치/권력/협상/책략/궁정/원로원 → **정치**
  - 일상/힐링/성장/학교/직장/시트콤 → **일상**
  - 로맨스/연애/멜로/로코 → **로맨스**
- 애매하면 기본값은 **일상**.

#### 2) 디폴트 약속 + 예외 토글 제시
선택된 카드의 “기본 약속” 4~7줄과 “예외 설정(토글)”을 한 화면에 보여준다.

출력 템플릿(예시):

> 이번 세션은 이런 ‘장르 약속’으로 굴릴게.
> - 약속 카드: {카드 이름}
> - 기본 약속:
>   1) ...
>   2) ...
>   3) ...
> - 예외 설정(바꾸고 싶으면 항목만 적어줘):
>   - {토글A}: {값}
>   - {토글B}: {값}
>
> 그대로면 "OK", 바꾸고 싶으면 토글만 적어줘.

#### 3) 확정 및 기록
- 플레이어가 "OK"면 디폴트 값으로 확정한다.
- 토글 수정이 오면, 해당 항목만 반영해 확정한다.
- 확정된 결과를 `wf.pm`에 기록하고, 이후 세션 동안 변경하지 않는다.
- 확정이 끝나면 **PC 생성 단계로 넘어간다.**

`wf.pm` 기록 규칙:
- `ct`: 카드 이름(추리·수사/정치/일상/로맨스)
- `tg`: 토글 딕셔너리(카드별로 필요한 것만)
- `pr`: 플레이어에게 보여준 “기본 약속” 문장을 그대로 줄 배열로 저장(자기검증용)

#### 카드별 기본값(권장)
- **추리·수사**
  - 기본 약속: 범인 있음, PC는 수사 주도, 공정 단서, 완결 해답, 우연 해결 금지
  - 토글 기본값: `culprit=ON`, `pc_role=탐정`, `ending=완결`, `supernatural=금지`, `luck=낮음`
- **정치**
  - 기본 약속: 이해관계 중심, 세력 선제 행동, 거래/배신 허용, 판세 축적
  - 토글 기본값: `cruelty=보통`, `mode=혼합(공개+암투)`, `war=가능`, `public=보통`
- **일상**
  - 기본 약속: 고강도 비극 금지(기본), 소소한 갈등, 작은 성취 보장, 회복 가능 톤
  - 토글 기본값: `realism=리얼`, `drama=잔잔`, `romance=조금`, `timeskip=보통`
- **로맨스**
  - 기본 약속: 감정/선택 중심, 관계 진전 보장, 문제행동 미화 금지, 결말은 한 단계 변화
  - 토글 기본값: `ending=해피`, `pace=보통`, `triangle=금지`, `maturity=순정`

> 주의: 플레이어에게 보여줄 때는 JSON 키(`culprit`, `ending` 등)를 그대로 쓰지 말고, 한국어 항목명으로 보여줘.
