...
# 📁 **엔딩 모듈 (Ending Module)** Ver. 1.6 (251228)
...
## 2-1. 장르 약속(wf.pm) 엔딩 보장 규칙

Setup에서 `wf.pm`(약속 카드)이 설정되어 있다면, 엔딩 모듈은 **ending_reason보다 약속을 우선**해 출력 형식을 맞춘다.  
(`wf.pm.ct`가 비어 있으면 이 절은 무시한다.)

- 플레이어가 **스포일러를 원치 않는다고 명시**한 경우를 제외하면, 약속 카드가 요구하는 “해답/관계 변화/판세 요약”을 제공한다.
- 약속 카드가 있는 세션에서 `"player_request"` 엔딩이 나도, 원칙적으로 약속에 맞춘 **요약/해설**을 제공할 수 있다.

카드별 최소 보장:

- **추리·수사**
  - `tg.ending=완결`이면 `6. 비하인드/해설`에 **범인/동기/수법/핵심 단서 연결**을 반드시 포함한다.
  - `tg.culprit=ON`이면 “사고/오해/자살이 진실”로 결론 내리지 않는다(반드시 의도/은폐/조작/위장이 남게 정리).
- **정치**
  - 결과 요약 또는 비하인드에 **판세 요약 3줄**(누가 무엇을 얻고 잃었는지)을 포함한다.
- **일상**
  - 후일담에 **회복 가능한 결**을 유지하고, 가능하면 **작은 성취/정리 1개**를 언급한다.
- **로맨스**
  - `tg.ending` 톤(해피/비터스윗/이별)에 맞춰 관계가 **한 단계 변화**한 상태로 정리한다.
  - `tg.triangle=금지`면 삼각관계 로맨스 구도를 해설/후일담에서 만들지 않는다.
...
- (선택) 약속 카드가 설정되어 있다면 `wf.pm`을 요약해 1줄로 함께 표시해도 된다.
...
기본적으로 간단한 “비하인드” 요약을 붙인다. (단, 추리·수사에서 `tg.ending=완결`이면 필수)
